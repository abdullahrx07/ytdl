...
async function waitForDownload(
  ...
  for (let attempt = 0; attempt < MAX_PROGRESS_POLLS; attempt += 1) {
    ...
    if (
      (typeof result.error === "number" && result.error > 0) ||
      (typeof result.error === "string" && result.error !== "0")
    ) {
      return result;
    }

    if (typeof result.progress === "number" && result.progress >= 3) {
      return {
        ...result,
        title: result.title ?? initialResult.title,
        downloadURL: initialResult.downloadURL ?? result.downloadURL,
      };
    }

    if (result.downloadURL && result.progress === undefined) {
      return result;
    }
  }
